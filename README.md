...
- В форме добавления/редактирования шортката есть обязательный чекбокс согласия и ссылки на юридические документы.
...
- Управление пользовательскими данными из меню `Settings`:
  - `Privacy` (отдельное подменю с тумблерами внешних функций)
...
`Privacy`
- Открывает отдельное подменю конфиденциальности.
- В подменю доступны тумблеры:
  - поисковые подсказки (`Google Suggest`);
  - виджет погоды (внешний API);
  - виджет валют (внешний API);
  - IP/регион виджет (внешний API).
...
- На проекте добавлены отдельные страницы:
  - `./privacy.html` — Политика конфиденциальности;
  - `./cookies.html` — Cookies и localStorage.
  - `./agreement.html` — Пользовательское соглашение.
- При первом посещении показывается privacy-баннер с кнопкой `Согласен`.
  - Нажатие `Согласен` включает все privacy-тумблеры и сохраняет согласие.
- По умолчанию (до согласия) privacy-тумблеры выключены.
- Необязательные внешние функции можно включать/отключать в подменю `Privacy`:
  - Google Suggest;
  - виджет погоды;
  - виджет валют;
  - IP/регион виджет.
- В подменю `Privacy` размещены ссылки на юридические документы:
  - Политика конфиденциальности;
  - Cookies и localStorage;
  - Пользовательское соглашение.
- В форме `shortcutForm` добавлен обязательный чекбокс согласия (по умолчанию снят) с ссылками на `privacy.html`, `cookies.html` и `agreement.html`.
- Проект использует:
  - `localStorage` для пользовательских настроек;
  - Service Worker/Cache Storage для статических ассетов.
- Проект полностью статический и совместим с GitHub Pages, все внутренние ссылки и ассеты используют относительные пути (`./...`).
